...
interface Emits {
  (e: 'close'): void
  (e: 'save', data: { clientId: number, items: LineItem[], templateId?: number, payByDays: number, vatInclusive: boolean }): void
}
...
const invoiceItems = ref<LineItem[]>([{ description: '', amount: 1, pricePerUnit: 0 }])
...
const totalInvoiceAmount = computed(() =>
  invoiceItems.value.reduce((sum, item) => sum + (item.amount * item.pricePerUnit), 0)
)

const orgVatRatePercent = computed(() => (organizationStore.currentOrganization as any)?.vatRate ?? 15)

const vatAmount = computed(() =>
  vatInclusive.value ? 0 : totalInvoiceAmount.value * (orgVatRatePercent.value / 100)
)

const totalWithVat = computed(() => totalInvoiceAmount.value + vatAmount.value)

const computedPayByDate = computed(() => {
  const date = new Date()
  date.setDate(date.getDate() + (payByDays.value || 30))
  return date.toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' })
})

// Map clients to DropdownItem shape
...
// Map templates to DropdownItem shape
  ...
  <Dialog :open="show" @update:open="(open) => !open && handleClose()">
    <DialogContent class="sm:max-w-2xl max-h-[90vh] overflow-hidden flex flex-col" :prevent-close="true">
      ...
      <div class="space-y-4 overflow-y-auto flex-1 pr-2">
        ...
        <!-- Pay By Days -->
        <div>
          <Label for="inv-pay-by-days">Days until due *</Label>
          <div class="flex items-center gap-3 mt-1">
            ...
            />
            <span class="text-sm text-gray-500">Due date: {{ computedPayByDate }}</span>
          </div>
        ...
        <div class="pt-3 border-t border-gray-200">
          <div class="flex justify-between items-center">
            ...
            <span class="text-sm text-gray-600">R {{ totalInvoiceAmount.toFixed(2) }}</span>
          </div>
      ...
      <DialogFooter>
        ...
        <Button @click="handleSave" variant="default">Create Invoice</Button>
      </DialogFooter>
    </DialogContent>
  </Dialog>
</template>
