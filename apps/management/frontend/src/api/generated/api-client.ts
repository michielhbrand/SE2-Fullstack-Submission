...
export class ManagementApiClient {
    ...
    seedDemoData(id: number, cancelToken?: CancelToken): Promise<void> {
        let url_ = this.baseUrl + "/api/v1/organizations/{id}/seed-demo-data";
        if (id === undefined || id === null)
            throw new globalThis.Error("The parameter 'id' must be defined.");
        url_ = url_.replace("{id}", encodeURIComponent("" + id));
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.instance.request(options_).catch((_error: any) => {
            ...
            return this.processSeedDemoData(_response);
        });
    }

    protected processSeedDemoData(response: AxiosResponse): Promise<void> {
        const status = response.status;
        let _headers: any = {};
        if (response.headers && typeof response.headers === "object") {
            for (const k in response.headers) {
